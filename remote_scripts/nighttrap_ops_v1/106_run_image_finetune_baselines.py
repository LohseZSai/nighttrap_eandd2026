...
TASK_ROOTS = {
    "empty": "results/nighttrap_ops_v1_build/track_a_empty",
    "species": "results/nighttrap_ops_v1_build/track_b_species",
    "count": "results/nighttrap_ops_v1_build/track_c_count",
    "usability": "results/nighttrap_supervised_baselines/clip_linear_probe_v1/usability",
}
...
def build_model(model_name: str, n_classes: int) -> tuple[nn.Module, Any, str]:
    if model_name == "vit_b16":
        weights = ViT_B_16_Weights.IMAGENET1K_V1
        model = vit_b_16(weights=weights)
        model.heads.head = nn.Linear(model.heads.head.in_features, n_classes)
        return model, weights.transforms(), "ViT-B/16 supervised fine-tune"
    if model_name == "convnext_tiny":
        weights = ConvNeXt_Tiny_Weights.IMAGENET1K_V1
        model = convnext_tiny(weights=weights)
        in_features = model.classifier[2].in_features
        model.classifier[2] = nn.Linear(in_features, n_classes)
        return model, weights.transforms(), "ConvNeXt-Tiny supervised fine-tune"
...
def evaluate(
    ...
    with torch.no_grad():
        for images, _, ids, labels in tqdm(loader, desc="eval", leave=False):
...
def write_outputs(
    ...
    summary = {
        ...
        "top_confused_pairs": confusions[:20],
        "extra": summary_extra,
        "outputs": {
...
def main() -> None:
    ...
    parser.add_argument("--task", choices=sorted(TASK_ROOTS), required=True)
    parser.add_argument("--model", choices=["vit_b16", "convnext_tiny"], required=True)
    ...
    root = Path(args.root)
    task_root = root / (args.task_root or TASK_ROOTS[args.task])
    out_dir = root / (
        args.out_dir
        or f"results/nighttrap_supervised_baselines/{args.model}_finetune_{args.task}_v1"
    )
    train_items = load_json(task_root / "train.json")
    dev_items = load_json(task_root / "dev.json")
    test_items = load_json(task_root / "test.json")
    labels = sorted({choice_label(x) for x in train_items})
    label_to_idx = {label: i for i, label in enumerate(labels)}
    model, transform, model_label = build_model(args.model, len(labels))

    train_ds = ImageTaskDataset(train_items, label_to_idx, transform)
    dev_ds = ImageTaskDataset(dev_items, label_to_idx, transform)
    test_ds = ImageTaskDataset(test_items, label_to_idx, transform)
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        losses = []
        for images, targets, _, _ in tqdm(train_loader, desc=f"{args.model}/{args.task} epoch {epoch}"):
            images = images.to(device, non_blocking=True)
