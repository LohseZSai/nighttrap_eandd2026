...
def context(row: dict[str, Any]) -> dict[str, float]:
    ...
    feats: dict[str, float] = {
        ...
        "species_ref_count": float(ref.get("species_ref_count_audit_only") or 0),
        "species_ref_ratio": float(ref.get("species_ref_ratio_audit_only") or 0),
        "month_neighbor_support": float(ref.get("month_neighbor_support_audit_only") or 0),
    ...
    for key, value in flags.items():
        feats[f"audit_flag={key}"] = float(bool(value))
    return feats


def context_matrix(train, dev, test):
    vec = DictVectorizer(sparse=False)
    x_train = vec.fit_transform([context(row) for row in train]).astype("float32")
    x_dev = vec.transform([context(row) for row in dev]).astype("float32")
    x_test = vec.transform([context(row) for row in test]).astype("float32")
...
def auto_pass_at_recall(y: np.ndarray, score: np.ndarray, target_recall: float = 0.95) -> dict[str, Any]:
    ...
    if positives == 0:
        return best
    # Low scores are auto-passed; high scores are reviewed.
    for threshold in np.unique(score):
        ...
        if recall >= target_recall and int(auto.sum()) > best["auto_pass_n"]:
            best = {
                "auto_pass_n": int(auto.sum()),
                "auto_pass_rate": float(auto.mean()),
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="results/nighttrap_supervised_baselines/needs_review_tree_ranker_variants_v1")
    parser.add_argument("--table", default="results/nighttrap_tables/table_needs_review_tree_rankers_v1.tex")
    ...
    x_ctx_train, x_ctx_dev, x_ctx_test, vectorizer = context_matrix(train, dev, test)

    # Scale CLIP features for tree models only to keep numeric ranges comparable.
    ...
    variants = {
        ...
        "context-lite-only": (x_ctx_train, x_ctx_dev, x_ctx_test),
        "CLIP+context-lite": (
    ...
    lines = [
        "\\begin{table*}[h]",
        "\\centering",
        "\\caption{Tree-based needs-review ranker variants on the frozen 913-event CLIP/context-lite split. All rows use the fixed routine $\\rightarrow$ 0, review/priority-review $\\rightarrow$ 1 mapping. Values are percentages except enrichment.}",
        "\\label{tab:needs_review_tree_rankers}",
        "\\scriptsize",
